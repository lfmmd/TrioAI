...
- **API 兼容格式**：[Anthropic Messages API](https://docs.anthropic.com/en/api/messages)（端点 `/v1/messages`，请求体/响应/SSE 流式事件均遵循 Anthropic 规范）。**不兼容 OpenAI Chat Completions 格式**。支持智谱 GLM（`GLM-5.2`、`GLM-5.1`、`GLM-5`）、DeepSeek、Anthropic 官方、以及任何兼容 Anthropic Messages API 的代理或第三方服务
...
|------|---------------------|----------|----------|
| **智谱 GLM** | `https://open.bigmodel.cn/api/anthropic` | `GLM-5.2`、`GLM-5.1`、`GLM-5`、`GLM-4.6` | https://bigmodel.cn |
...
TrioAI 内置约 22 MB 的 TrioBASIC / IEC / PLCopen 三大命令参考库（打包压缩后；解压约 36 MB），AI 通过 `lookup_command` 工具实时查证命令语法、避免幻觉。**这些数据需要在使用前手动初始化一次**：
...
**初始化做了什么：**
- 把插件目录下的 `skills\`（triobasic / iec / plcopen / general 四个库，压缩包共约 22 MB）复制到 `%APPDATA%\TrioAI\skills\`
